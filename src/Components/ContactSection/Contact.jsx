...
const Contact = ({ aboutText }) => {
  const heroDisplaySettings =
    "hidden md:block h-full md:hero lg:col-span-3 lg:row-span-4 rounded-3xl";

  const socialCustomClasses =
    "order-1 md:order-none  grid grid-cols-2 grid-rows-2 socials gap-2 lg:flex lg:flex-col lg:items-center lg:justify-center lg:col-span-1 lg:row-span-4 lg:gap-4";

  return (
    <section className="flex flex-col gap-4 justify-center py-10 pb-40 p-4 md:p-0 h-full w-full md:grid  md:tablet-grid-contact lg:grid-cols-9 lg:grid-rows-6 lg:gap-4 relative">
